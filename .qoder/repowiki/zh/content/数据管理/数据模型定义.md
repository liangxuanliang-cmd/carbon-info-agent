...
## 更新摘要
**变更内容**
- 政策数据模型的regionType从'national' | 'province' | 'city'简化为'national' | 'city'
- category从'policy' | 'methodology'简化为'policy'
- 反映了新的地理区域分类系统和政策分类体系
- 更新了相关的类型定义、数据转换逻辑和组件实现
...
**更新** 项目现已支持简化的政策数据模型，regionType仅包含'national' | 'city'两个层级，category统一为'policy'，反映了新的地理区域分类系统和政策分类体系。
...
C4["scripts/crawler/policyCrawler.ts<br/>政策列表爬虫"]
end
subgraph "工具层"
...
- [src/data/policies.ts:1-232](file://src/data/policies.ts#L1-L232)
...
- [scripts/crawler/policyCrawler.ts:1-247](file://scripts/crawler/policyCrawler.ts#L1-L247)
...
### Policy（政策）
**更新** regionType从'national' | 'province' | 'city'简化为'national' | 'city'，category从'policy' | 'methodology'简化为'policy'

- 字段与类型
  - id: string（主键标识）
  - title: string（标题）
  - regionType: 'national' | 'city'（区域层级，简化后的枚举）
  - province: string（省/市名称）
  - category: 'policy'（政策类别，统一为政策）
...
- 约束与校验
  - 唯一键：id
  - 枚举值：regionType（national、city）、category（policy）、status（active、expired）
  - 可选字段：replacedBy
  - 发布日期格式：字符串日期，建议统一为ISO 8601
- 业务含义
  - 描述碳普惠相关的政策文件，支持按区域、状态过滤与展示。
- 关系
  - 与CarbonProduct无直接外键关联
  - 由PolicyDetailData转换而来，增加了regionType、province、category等字段映射

**更新** 简化了regionType和category的枚举值，移除了province层级，统一为policy类别。
...
## 结论
本项目通过清晰的类型定义与静态数据组织，实现了政策、方法学、新闻与碳价等核心数据模型的稳定展示与交互。简化的政策数据模型（regionType: 'national' | 'city'，category: 'policy'）提高了数据模型的一致性和易用性。类型安全与TS推导在组件中得到良好运用，配合useMemo优化提升了性能。建议后续在数据量增长时引入服务端与缓存机制，并持续完善类型约束与错误处理。

## 附录

### 枚举与常量
- 区域类型：national、city（简化后的枚举，移除了province）
- 政策类别：policy（统一为政策类别，移除了methodology）
- 政策状态：active、expired
- 碳产品市场：domestic、international
- 政策层级：national、city（简化后的枚举）

**更新** 简化了区域类型和政策类别的枚举值，移除了province层级和methodology类别。
