...
H[scripts/crawler/]
I[scripts/utils/]
J[scripts/autoUpdate.ts]
end
subgraph "数据存储层"
K[src/data/]
L[scripts/updateData.ts]
end
A --> B
B --> C
D --> A
...
**章节来源**
- [package.json:1-40](file://package.json#L1-L40)
- [vercel.json:1-42](file://vercel.json#L1-L42)
- [vite.config.ts:1-8](file://vite.config.ts#L1-L8)
...
系统集成了完整的自动化部署流程，支持Vercel平台的无缝部署：
...
系统采用前后端分离的现代化架构，前端使用React单页应用，后端采用无服务器函数和自动化脚本：
