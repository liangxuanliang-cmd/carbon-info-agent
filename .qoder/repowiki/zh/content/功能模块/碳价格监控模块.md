...
**更新** 本模块现已集成全新的百度搜索碳价格爬虫系统，替代原有的专业碳价爬虫，实现了基于搜索引擎的智能价格提取和识别功能。
...
RenderDomestic --> Combine["组合输出"]
SkipDomestic --> Combine
RenderInternational --> Combine
SkipInternational --> Combine
```
...
**章节来源**
- [PriceTable.tsx:1-81](file://src/sections/PriceTable.tsx#L1-L81)
...
CreateRecord --> ReturnData["返回完整数据集"]
ReturnData --> End([结束])
```
...
MergeData --> DomesticTrend
MergeData --> InternationalTrend
```
...
DefinePatterns --> Pattern2["模式2: 收盘价<br/>收盘价[：:]:\\s*(\\d+\\.?\\d+).*?([+-]?\\d+\\.?\\d*)%"]
DefinePatterns --> Pattern3["模式3: 最新价<br/>最新价[：:]:\\s*(\\d+\\.?\\d+).*?([+-]?\\d+\\.?\\d*)%"]
...
未来可以考虑的功能增强包括：实时数据接入、更复杂的价格分析算法、移动端优化、多搜索引擎支持等。
